...
const YouTubeDashboard = () => {
  ...
  useEffect(() => {
    try {
      const savedData = localStorage.getItem('youtubeDashboardData');
      if (savedData) {
  ...
  // Save data to localStorage whenever youtubeData changes
  useEffect(() => {
    if (youtubeData.ageData || youtubeData.genderData || youtubeData.subscriptionData || 
        youtubeData.contentData || youtubeData.geographyData) {
      try {
        localStorage.setItem('youtubeDashboardData', JSON.stringify(youtubeData));
      } catch (error) {
  ...
  const exportDashboardData = () => {
    try {
      ...
      const dataStr = JSON.stringify(youtubeData);
      const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
      
      // Create a download link and trigger it
      const exportFileDefaultName = `youtube-dashboard-export-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const clearDashboardData = () => {
    if (window.confirm('Are you sure you want to clear all dashboard data? This cannot be undone.')) {
      ...
      localStorage.removeItem('youtubeDashboardData');
