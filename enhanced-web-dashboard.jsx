...
const WebDashboard = () => {
  ...
  useEffect(() => {
    try {
      const savedData = localStorage.getItem('webDashboardData');
      if (savedData) {
  ...
  // Save data to localStorage whenever webData changes
  useEffect(() => {
    if (webData.demographics || webData.trafficAcquisition || 
        webData.pagesAndScreens || webData.utms) {
      try {
        localStorage.setItem('webDashboardData', JSON.stringify(webData));
      } catch (error) {
  ...
  const exportDashboardData = () => {
    try {
      ...
      const dataStr = JSON.stringify(webData);
      const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
      
      // Create a download link and trigger it
      const exportFileDefaultName = `web-dashboard-export-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const clearDashboardData = () => {
    if (window.confirm('Are you sure you want to clear all dashboard data? This cannot be undone.')) {
      ...
      localStorage.removeItem('webDashboardData');
