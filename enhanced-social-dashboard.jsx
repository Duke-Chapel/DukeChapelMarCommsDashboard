...
const SocialDashboard = () => {
  ...
  useEffect(() => {
    try {
      const savedData = localStorage.getItem('socialDashboardData');
      if (savedData) {
        const parsedData = JSON.parse(savedData);
        if (parsedData && (parsedData.facebook || parsedData.instagram)) {
  ...
  // Save data to localStorage whenever socialData changes
  useEffect(() => {
    if (socialData.facebook || socialData.instagram) {
      try {
        localStorage.setItem('socialDashboardData', JSON.stringify(socialData));
      } catch (error) {
  ...
  const exportDashboardData = () => {
    try {
      ...
      const dataStr = JSON.stringify(socialData);
      const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
      
      // Create a download link and trigger it
      const exportFileDefaultName = `social-dashboard-export-${new Date().toISOString().split('T')[0]}.json`;
  ...
  const importDashboardData = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (importedData && (importedData.facebook || importedData.instagram)) {
          setSocialData(importedData);
  ...
  const clearDashboardData = () => {
    if (window.confirm('Are you sure you want to clear all dashboard data? This cannot be undone.')) {
      ...
      localStorage.removeItem('socialDashboardData');
